...
| `KEYCLOAK_JWKS_CACHE_TTL_SECONDS` | - | `300` | JWKS 캐시 유지 시간(초) |
...
인증 방식은 **service account** (`KEYCLOAK_ADMIN_CLIENT_ID` + `KEYCLOAK_ADMIN_CLIENT_SECRET`) 또는
**관리자 사용자명/비밀번호** (`KEYCLOAK_ADMIN_USERNAME` + `KEYCLOAK_ADMIN_PASSWORD`) 중 하나만.
...
- SQLite 파일 경로는 상대 경로를 허용하며, 작업 디렉터리 기준으로 해석한다.
...
- `DOCMESH_ENV=integration`은 테스트 실행 환경을 식별하는 표준 패턴이다.
