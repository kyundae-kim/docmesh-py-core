...
@log_function_boundary()
def _run_service_check(service_name: str, check: CheckFn, *, timer: Callable[[], float]) -> tuple[ServiceHealthStatus, Exception | None]:
    start = timer()
    try:
        check()
        ...
        return (ServiceHealthStatus(service=service_name, ok=False, latency_ms=latency_ms, error=error), exc)
    latency_ms = int(round((timer() - start) * 1000))
    return (ServiceHealthStatus(service=service_name, ok=True, latency_ms=latency_ms), None)

@log_function_boundary()
def check_all_services(service_checks: Mapping[str, CheckFn], *, required_services: Set[str] | None=None, timer: Callable[[], float]=time.perf_counter, parallel: bool=False) -> HealthCheckResult:
